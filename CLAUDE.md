...
- Before adding a new UI pattern, check `src/components/ui/` and `src/components/` first, and reuse or extend an existing primitive rather than writing a one-off.
...
- Clean up any test data created in Supabase while verifying a flow, and don't leave scratch products/bills behind in the shared dev database.
...
- Respect the phase boundaries in `PROJECT_SPEC.md` section 8, and don't pull work forward from a later phase (e.g. PDF/share UI is Phase 4, full animation polish is Phase 8) even if it would be easy to bolt on while touching adjacent code.

## Writing style

- Never use an em dash (—) in prose, comments, or commit messages. Use a period, comma, or parentheses instead.
